...
- **When to fix:** Before any beta distribution (B-054): deploy the backend + approve the Twilio WhatsApp sender so real OTP delivery works for actual users (the bypass is debug-only and never reaches production).
