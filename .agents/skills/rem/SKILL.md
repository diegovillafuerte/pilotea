---
name: rem
description: Self-healing maintenance agent. Fixes PM drift, updates stale docs, syncs AGENTS.md, triages techdebt, and commits changes. Runs autonomously (nightly) or manually. Use /rem for full run, /rem quick for PM-only, /rem dry for preview.
...
## Phase 2: AGENTS.md Sync

### 2.1: Gather actual state

Run these in parallel:
- List all skill directories: `ls $MAIN_PATH/.Codex/skills/`
- List all command files: `find $MAIN_PATH/.Codex/commands/ -name "*.md" -type f`
- List all src top-level directories: `ls -d $MAIN_PATH/src/*/`
- Read current AGENTS.md: `$MAIN_PATH/AGENTS.md`
- Extract all backtick-quoted file paths from AGENTS.md and verify each exists

### 2.2: Detect drift

Compare:
- **Skills:** actual skills vs skills mentioned in AGENTS.md (the "Other skills" line and any skill tables)
- **Directories:** actual `src/` directories vs the project structure table in AGENTS.md
- **File refs:** backtick-quoted paths that don't exist on disk

### 2.3: Apply fixes

If any drift detected, spawn the **Codex-md-sync subagent** (read prompt from `$MAIN_PATH/.Codex/skills/rem/healers/Codex-md-sync.md`):
1. Replace `{CLAUDE_MD}` with current AGENTS.md content
...
- Diff the result against the original AGENTS.md
...
1. Read the prompt template from `$MAIN_PATH/.Codex/skills/rem/healers/doc-updater.md`
...
Compare against existing techdebt entries. For comments not yet tracked, spawn the **techdebt-triage subagent** (read prompt from `$MAIN_PATH/.Codex/skills/rem/healers/techdebt-triage.md`):
...
Read `$MAIN_PATH/../.Codex/projects/-Users-diegovillafuerte-ongoing-projects-pilotea/memory/MEMORY.md`. Note: the memory directory path may vary — if this path doesn't work, skip this step silently.
...
Check if any skill referenced in `$MAIN_PATH/docs/guides/agentic-development.md` doesn't exist in `.Codex/skills/` or `.Codex/commands/`. Flag discrepancies.
...
### AGENTS.md Sync
<list each change made to AGENTS.md>
...
1. **Never modify source code.** Rem only touches: `docs/`, `pming/`, `AGENTS.md`, `techdebt.md`, and its own report files. If you find yourself about to write to `src/`, `test/`, `admin/`, or any `.ts` file — STOP.
...
4. **AGENTS.md change cap.** Never apply more than 15 changed lines to AGENTS.md in a single run.
