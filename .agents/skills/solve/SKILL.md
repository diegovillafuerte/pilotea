...
> Solve-specific parameters: `OUTPUT_SCHEMA_PATH=.Codex/skills/solve/diagnosis-review-schema.json`, `TIMEOUT_MS=300000`, `MAX_ROUNDS=3`, `FALLBACK_BEHAVIOR=Codex-agent`, `ON_REJECT=revise-and-resubmit`.

### 4b.1: Check Codex availability

Per shared protocol Step 1. If `CODEX_MISSING`: warn the user ("Codex CLI not installed -- run `/codex:setup` to install and authenticate, or falling back to Codex review"), then skip to substep 4b.3a (Codex fallback per shared protocol Step 3a).

### 4b.2: Build the review prompt

Construct the prompt per shared protocol Steps 2.1 and 2.2. Do NOT paste the full contents of AGENTS.md or ARCHITECTURE.md -- Codex runs with read-only sandbox access to the repo and can read any file it needs.
...
Read `.Codex/codex-guide.md` FIRST — it explains how to efficiently explore this repo. Then, for this review:
1. Read `AGENTS.md` for project constraints and architecture rules
...
If the command fails (non-zero exit, timeout, malformed output): warn the user ("Codex review failed -- falling back to Codex review"), then go to substep 4b.3a.

### 4b.3a: Codex fallback review

Per shared protocol Step 3a (`FALLBACK_BEHAVIOR=Codex-agent`). Spawn a Codex subagent with the same review prompt from Step 4b.2, requesting JSON output matching the diagnosis-review schema. Continue to Step 4b.4 with the parsed result.
