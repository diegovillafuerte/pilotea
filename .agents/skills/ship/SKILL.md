...
- Read documentation files for the reviewers' context: `ARCHITECTURE.md`, `AGENTS.md`, `techdebt.md`. If any flows were changed, also read the relevant flow's `knowledge-base.md` and `tone.md`.
...
> **Shared Codex protocol:** `.Codex/skills/_shared/codex-review-protocol.md` defines the reusable execution skeleton (availability check, prompt assembly, execution, parsing, reconciliation, fallback). This section specifies ship-specific parameters and orchestration logic.
...
The lean lane runs a single condensed review using the Codex CLI (or Codex fallback) with the lean reviewer prompt (`.Codex/skills/ship/reviewers/lean.md`). This covers security, architecture, and correctness in one pass with a shorter checklist, skipping the parallel specialized reviewer battery.
...
1. Read `.Codex/skills/ship/reviewers/lean.md`
...
codex exec - \
  --sandbox read-only \
  --output-schema .Codex/skills/ship/code-review-schema.json \
...
> **Codex execution follows the shared protocol in `.Codex/skills/_shared/codex-review-protocol.md`.**
> Ship-specific parameters: `OUTPUT_SCHEMA_PATH=.Codex/skills/ship/code-review-schema.json`, `TIMEOUT_MS=480000`, `MAX_ROUNDS=1`, `FALLBACK_BEHAVIOR=warn-and-proceed`, `ON_REJECT=show-and-ask-user`.

For each reviewer to run:

1. Read the reviewer's prompt file from `.Codex/skills/ship/reviewers/<name>.md`
...
4. Prefix each assembled prompt with the navigation header from the shared protocol (Step 2.2):
   ```
   You have read-only access to the full repo. Read `.Codex/codex-guide.md` for efficient navigation.
...
- If the `codex` CLI is not installed or not authenticated, warn the user and suggest running `/codex:setup`. Do NOT fall back to Codex agents — the user has opted for Codex-based reviews to preserve Codex tokens.
