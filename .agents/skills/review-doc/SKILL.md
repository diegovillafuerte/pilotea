---
name: review-doc
description: Structure Codex's response as a reviewable document in the Canvas app instead of dumping it into chat. Writes current.json, waits for review.json, feeds structured feedback back.
...
1. Create the canvas directory if it doesn't exist:
   ```bash
   mkdir -p ~/.Codex/canvas
   ```

2. Delete any stale `review.json` from a previous session:
   ```bash
   rm -f ~/.Codex/canvas/review.json
   ```

3. Write the document. Use the Write tool to create `~/.Codex/canvas/current.json` with the JSON from Step 1. Ensure the JSON is valid and pretty-printed.
...
open -a "Codex Canvas" 2>/dev/null || true
```
...
- Never modify files outside `~/.Codex/canvas/` — this skill only reads/writes `current.json` and `review.json`
