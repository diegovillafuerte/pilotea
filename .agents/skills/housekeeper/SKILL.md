...
## Step 2: AGENTS.md Sharpness Audit

AGENTS.md is the front door for every agent session. It must be razor-sharp: every instruction currently true, every reference pointing to something that exists, every description matching reality. A stale AGENTS.md actively misleads agents and causes wasted work.

### CMD-1: File references exist

Extract every file path and directory mentioned in AGENTS.md (look for backtick-quoted paths like `TASKS.md`, `BLUEPRINT.md`, `docs/...`, `src/...`, `pming/...`, `techdebt.md`). For each, verify the file/directory exists on disk.
...
1. Extract the fenced code block under `## Project structure` in AGENTS.md
...
Read the first paragraph of AGENTS.md. Check:
...
3. Skills mentioned actually exist in `.Codex/skills/`
...
2. Read the "Custom lint rules" section in AGENTS.md
...
Count lines in AGENTS.md. Warn if > 300 (research shows instruction quality degrades beyond this).
- Severity: `[i]` advisory
...
AGENTS.md:        N critical, N warnings, N advisories
