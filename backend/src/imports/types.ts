/**
 * Import-pipeline types and Claude Vision extraction schemas.
 *
 * Ported verbatim from the legacy web app (src/lib/parsers/types.ts). The Zod
 * schemas and ParsedMetrics shape are the production-proven asset — they are
 * reproduced exactly so the ported parsers behave identically. Only the import
 * path for zod (v4 in the backend) differs from the web app.
 */
...
export const indriveScreenshotExtractionSchema = z.object({
  week_start: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "week_start debe ser una fecha ISO valida (YYYY-MM-DD)")
    .nullable(),
  net_earnings: z.number().nullable(),
  gross_earnings: z.number().nullable(),
  total_trips: z.number().int().nullable(),
