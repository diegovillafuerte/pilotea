/**
 * DiDi dual-screenshot parser (earnings screen + tablero/dashboard). Ported
 * verbatim from the web app (src/lib/parsers/didi-screenshot.ts) — the 2-image
 * requirement and its Spanish error, the prompts, the derived hours/trips maths,
 * and the week_start fallback are reproduced exactly. The Claude client is
 * injected so tests replay fixtures without an API key.
 */

import type { VisionClient } from "../claude.js";
import { extractJsonFromResponse } from "../claude.js";
import type { ParseInput, ParseResult, ParsedMetrics, DidiScreenshotExtraction } from "../types.js";
import { didiScreenshotExtractionSchema } from "../types.js";
import { calculateDataCompleteness, prepareFileForVision, getCurrentMonday } from "../utils.js";
...
function deriveTripsPerHour(totalTrips: number | null, hoursOnline: number | null): number | null {
...
// ─── Main parser ──────────────────────────────────────────────
export async function parseDidiScreenshot(
  input: ParseInput,
  vision: VisionClient,
): Promise<ParseResult> {
  const [rawFile0, rawFile1] = input.files;
  if (!rawFile0 || !rawFile1) {
    return {
      success: false,
      metrics: null,
      raw_extraction: {},
  ...
  try {
    const [file0, file1] = await Promise.all([
      prepareFileForVision(rawFile0, input.mimeType),
      prepareFileForVision(rawFile1, input.mimeType),
    ]);

    const response = await vision.callClaudeVision({
