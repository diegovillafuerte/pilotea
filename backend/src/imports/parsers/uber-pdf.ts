/**
 * Uber weekly PDF parser. Ported verbatim from the web app
 * (src/lib/parsers/uber-pdf.ts) — system/user prompts, derived-metric maths,
 * Spanish error strings, and the `week_start ?? getCurrentMonday()` fallback are
 * all reproduced exactly. The only structural change: the Claude client is
 * injected (VisionClient) instead of imported from a module singleton, so tests
 * can replay fixtures with no API key.
 */

import type { VisionClient } from "../claude.js";
import { extractJsonFromResponse } from "../claude.js";
import type { ParseInput, ParseResult, ParsedMetrics, UberPdfExtraction } from "../types.js";
import { uberPdfExtractionSchema } from "../types.js";
import { calculateDataCompleteness, getCurrentMonday } from "../utils.js";
...
function calculateDerivedMetrics(
  ...
  ParsedMetrics,
  "earnings_per_trip" | "earnings_per_hour" | "trips_per_hour" | "earnings_per_km" | "total_km"
> {
...
// ─── Main parser ──────────────────────────────────────────────
export async function parseUberPdf(input: ParseInput, vision: VisionClient): Promise<ParseResult> {
