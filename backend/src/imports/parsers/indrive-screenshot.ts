/**
 * InDrive single-screenshot parser. Ported verbatim from the web app
 * (src/lib/parsers/indrive-screenshot.ts) — prompts, Spanish errors, the
 * commission/earnings-per-trip derivations, and the week_start fallback are
 * reproduced exactly. The Claude client is injected so tests replay fixtures
 * without an API key.
 */
...
import type {
  ...
  ParseResult,
  ParsedMetrics,
  IndriveScreenshotExtraction,
} from "../types.js";
import { indriveScreenshotExtractionSchema } from "../types.js";
...
// ─── Main parser ──────────────────────────────────────────────
export async function parseIndriveScreenshot(
  input: ParseInput,
  vision: VisionClient,
): Promise<ParseResult> {
  const [rawFile] = input.files;
  if (!rawFile) {
    return {
      success: false,
      metrics: null,
  ...
  try {
    const file = await prepareFileForVision(rawFile, input.mimeType);
