/**
 * Uber pie-chart screenshot parser. Ported verbatim from the web app
 * (src/lib/parsers/uber-screenshot.ts). Prompts, Spanish errors, the limited
 * field set, and the week_start fallback are reproduced exactly; the Claude
 * client is injected so tests replay fixtures without an API key.
 */

import type { VisionClient } from "../claude.js";
import { extractJsonFromResponse } from "../claude.js";
import type { ParseInput, ParseResult, ParsedMetrics, UberScreenshotExtraction } from "../types.js";
import { uberScreenshotExtractionSchema } from "../types.js";
...
// ─── Main parser ──────────────────────────────────────────────
export async function parseUberScreenshot(
  input: ParseInput,
  vision: VisionClient,
): Promise<ParseResult> {
  ...
  try {
    const prepared = await prepareFileForVision(file, input.mimeType);
