...
/**
 * Strips the driver-facing Kompara chip's own pixels out of an OCR frame before parsing.
 *
 * MediaProjection mirrors the WHOLE screen, so a captured frame contains our floating verdict chip
 * composited on top of the host offer. The chip prints bare-"$" rates ("$15.00/km", "$450/h") that
 * [DidiOcrParser]'s fare regex happily matches, so the chip's own number gets read as the offer's
 * fare — a self-capture feedback loop that drove the live verdict to $0 (the displayed value feeds
 * back in as the next frame's fare and collapses). [mx.kompara.overlay] publishes the chip's current
 * rect to [mx.kompara.data.service.ScreenReaderState.overlayChipRect]; this drops every OCR line that
